	...
	<div class="container">
		<div class="product-sec1 border border-danger" style="margin-top:10px; border-radius:10px">
			<ul class="w3_short">
                ...
                <?php
                    $sql_newsname = mysqli_query($con,"SELECT * FROM tbl_newsfeed WHERE newsfeed_id='$newsfeed_id'");
                    $row_nname = mysqli_fetch_array($sql_newsname);  
                ?>
                <li class="font-weight-bold"><?php echo $row_nname['newsfeed_name'] ?></li>
			</ul>
		</div>
	</div>
</div>
...
<!-- about -->
    <div class="welcome py-sm-5 py-4">
		<div class="container py-xl-4 py-lg-2">
            <?php
                $sql_newsfeed = mysqli_query($con,"SELECT * FROM tbl_newsfeed WHERE newsfeed_id='$newsfeed_id'");
                $row_nnamefeed = mysqli_fetch_array($sql_newsfeed);  
            ?>
			<!-- tittle heading -->
			<h4 class=" text-center mb-lg-5 mb-sm-4 mb-3 font-weight-bold" style="margin-top:-60px;"><?php echo $row_nnamefeed['newsfeed_name'] ?></h4>
			<!-- //tittle heading -->
            <?php
                $sql_post = mysqli_query($con,"SELECT * FROM tbl_newsfeed,tbl_post WHERE tbl_newsfeed.newsfeed_id = tbl_post.newsfeed_id AND tbl_newsfeed.newsfeed_id = '$newsfeed_id'");
